...
class MLPPlanner(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Input: (b, n_track * 2 + n_track * 2) = (b, 40) for n_track=10
        x = torch.cat([left_flat, right_flat], dim=1)
        
        # Pass through MLP
        x = self.fc(x)
        
        # Reshape to waypoints: (b, 6) -> (b, 3, 2) for n_waypoints=3
...
class TransformerPlanner(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Shape: (b, 2*n_track, 2)
        track_boundaries = torch.cat([track_left, track_right], dim=1)
        
        # Project input to d_model dimensions
        # Shape: (b, 2*n_track, d_model)
        memory = self.input_proj(track_boundaries)
        ...
        decoded = self.transformer_decoder(query_pos, memory)
...
class CNNPlanner(torch.nn.Module):
    def __init__(
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, n_waypoints * 2)  # Output: n_waypoints * 2 coordinates
        )
    ...
    def forward(self, image: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # Reshape to waypoints: (b, n_waypoints * 2) -> (b, n_waypoints, 2)
        waypoints = x.view(x.shape[0], self.n_waypoints, 2)
